...
def check_list(ex_name, f, exp_res, *args):
    ...
    if not len(res) == len(exp_res):
        log(
            red("FAIL"),
            ex_name,
            ": expected a list of size ",
            len(exp_res),
            " but got list of size",
            len(res),
        )
        return True
    if not all(equals(x, y) for x, y in zip(res, exp_res)):
...
def check_get_order():
    ...
    if check_list(ex_name, p1.get_order, [0], 1):
        log(
            "You should revert `get_order` to its original implementation for this test to pass"
        )
        return
...
def check_hinge_loss_single():
    ...
    exp_res = 1 - 0.8
    if check_real(
        ex_name, p1.hinge_loss_single, exp_res, feature_vector, label, theta, theta_0
    ):
        return
    log(green("PASS"), ex_name, "")
...
def check_hinge_loss_full():
    ...
    exp_res = 1 - 0.8
    if check_real(
        ex_name, p1.hinge_loss_full, exp_res, feature_vector, label, theta, theta_0
    ):
        return
...
def check_perceptron_single_update():
    ...
    if check_tuple(
        ex_name,
        p1.perceptron_single_step_update,
        exp_res,
        feature_vector,
        label,
        theta,
...
def check_average_perceptron():
    ...
    T = 1
    exp_res = (np.array([1, 2]), 1)
    if check_tuple(ex_name, p1.average_perceptron, exp_res, feature_matrix, labels, T):
        return
...
def check_classify():
    ...
    if check_array(ex_name, p1.classify, exp_res, feature_matrix, theta, theta_0):
        return

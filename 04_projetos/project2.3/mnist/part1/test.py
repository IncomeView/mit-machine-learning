...
def check_closed_form():
    ...
    if check_array(
        ex_name, linear_regression.closed_form, exp_res, X, Y, lambda_factor
    ):
        return
...
def check_svm():
    ...
    if check_array(ex_name, svm.one_vs_rest_svm, exp_res, train_x, train_y, test_x):
        return
...
def check_compute_probabilities():
    ...
    if check_array(ex_name, softmax.compute_probabilities, exp_res, X, zeros, temp):
        return
    ...
    exp_res[-1] = 1
    if check_array(ex_name, softmax.compute_probabilities, exp_res, X, theta, temp):
        return
...
def check_update_y():
    ...
    if check_tuple(ex_name, softmax.update_y, exp_res, train_y, test_y):
        return
    log(green("PASS"), ex_name, "")
...
def check_project_onto_PC():
    ...
    n_components = 2
    if check_array(
        ex_name, features.project_onto_PC, exp_res, X, pcs, n_components, feature_means
    ):
        return
    log(green("PASS"), ex_name, "")
        ...
        for j in range(m):
            ...
            if not equals(exp, got):
                log(
                    red("FAIL"),
                    ex_name,
                    ": values at ({}, {}) do not match. Expected {}, got {}".format(
                        i, j, exp, got
                    ),
                )
    log(green("PASS"), ex_name, "")
